/**
 * EJClaw OpenCode Runner
 *
 * Lightweight runner that sends prompts to OpenCode.ai API.
...
const OPENCODE_BASE_URL = (
  process.env.ANTHROPIC_BASE_URL ?? 'https://opencode.ai/zen'
).replace(/\/+$/, '');
const OPENCODE_MODEL = process.env.OPENCODE_MODEL ?? 'claude-opus-4-6';
const OPENCODE_API_KEY = process.env.ANTHROPIC_API_KEY ?? '';
...
async function callOpenCode(
  ...
  try {
    log(`Calling OpenCode at ${OPENCODE_BASE_URL} with model ${OPENCODE_MODEL}`);
    ...
    if (!response.ok) {
      ...
      throw new Error(`OpenCode HTTP ${response.status}: ${body.slice(0, 200)}`);
    }
...
async function main(): Promise<void> {
  ...
  log(`Role: ${role ?? 'none'}, model: ${OPENCODE_MODEL}`);
  ...
  // Attach git diff for reviewer so OpenCode can see actual file changes
  ...
  try {
    const responseText = await callOpenCode(systemPrompt, userPrompt);
    ...
    log(`OpenCode error: ${errorMessage}`);
    writeProtocolOutput<RunnerOutput>({
      status: 'error',
      result: null,
      error: `OpenCode call failed: ${errorMessage}`,
    });
    process.exit(1);
  }
}
